...
class BiDirectionalTrainer(object):
    def __init__(self, type, dataset, split, lr, diter, vis_screen, save_path, l1_coef, l2_coef,
        ...
        # image2text model
        self.txt2img_generator = torch.nn.DataParallel(
            Text2ImageGenerator(
                self.embed_size,
                self.hidden_size,
                len(self.vocab),
        ...
        self.txt2img_discriminator = torch.nn.DataParallel(
        ...
        # image2text model
        self.img2txt_generator = torch.nn.DataParallel(
            Image2TextGenerator(
                self.embed_size,
                self.hidden_size,
                len(self.vocab),
        ...
        self.img2txt_discriminator = torch.nn.DataParallel(
            Image2TextDiscriminator(self.embed_size, self.hidden_size, len(self.vocab), self.num_layers)
        )
        if is_cuda:
            self.txt2img_generator = self.txt2img_generator.cuda()
            self.txt2img_discriminator = self.txt2img_discriminator.cuda()
            self.img2txt_generator = self.img2txt_generator.cuda()
            self.img2txt_discriminator = self.img2txt_discriminator.cuda()

        if pre_trained_caption_gen and os.path.exists(pre_trained_caption_gen):
            print('loaded pretrained caption generator')
            self.img2txt_generator.load_state_dict(torch.load(pre_trained_caption_gen))

        if pre_trained_caption_disc and os.path.exists(pre_trained_caption_disc):
            print('loaded pretrained caption discriminator')
            self.img2txt_discriminator.load_state_dict(torch.load(pre_trained_caption_disc))
        ...
        self.img2txt_optimG = torch.optim.Adam(self.img2txt_generator.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
        self.img2txt_optimD = torch.optim.Adam(self.img2txt_discriminator.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
    ...
    def _pretrain_caption2image(self, cls, interp):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            for sample in tqdm(self.data_loader):
                ...
                self.img2txt_generator.zero_grad()
                sampled_captions, _ = self.img2txt_generator.forward(fake_images, right_captions, right_lengths)
            ...
            if (epoch + 1) % 10 == 0:
                ...
                torch.save(self.img2txt_discriminator.state_dict(), os.path.join(self.checkpoints_path, self.save_path,
                                                                                 'cycle_caption_disc-%d.pkl' % (
                                                                                             epoch + 1)))
                torch.save(self.img2txt_generator.state_dict(), os.path.join(self.checkpoints_path, self.save_path,
    ...
    def _train_stack_gan(self, cls, interp):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            ...
            # if (epoch+1) % 5 == 0:
                ...
                torch.save(self.img2txt_discriminator.state_dict(), os.path.join(self.checkpoints_path, self.save_path, 'cycle_caption_disc-%d.pkl' % (epoch + 1)))
                torch.save(self.img2txt_generator.state_dict(), os.path.join(self.checkpoints_path, self.save_path, 'cycle_caption_gen-%d.pkl' % (epoch + 1)))
